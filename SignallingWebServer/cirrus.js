...
var configFile = (typeof argv.configFile != 'undefined') ? argv.configFile.toString() : path.join(path.dirname(process.execPath), 'config.json');
let config = require('./modules/config.js').init(configFile, defaultConfig);
...
(() => {
	{
		if (config.UnrealProcessArgument) {
			config.UnrealProcessArgument.push(`-PixelStreamingPort=${config.StreamerPort}`);

		}

		// add platform param for windows and linux. they have different startup file.
		let getValue = (key) => {
			...
			for (let idx = 0; idx < process.argv.length; idx++) {
				...
				if (arg.startsWith(key)) {
					result = arg.substring(key.length, arg.length);
					break;
				}
			}
			return result;
		};
		config.Platform = getValue('platform=');
		console.log(`Current running platform is ${config.Platform}`);
	}
})();

console.log("Config: " + JSON.stringify(config, null, '\t'));
const ProcessManagerClass = require("./processManager");
const ProcessManager = new ProcessManagerClass(config);
...
if (config.UseHTTPS) {
	...
	// };
	const options = {
		key: fs.readFileSync(path.join(path.dirname(process.execPath), './certificates/client-key.pem')),
		cert: fs.readFileSync(path.join(path.dirname(process.execPath), './certificates/client-cert.pem'))
	};
...
if (config.UseAuthentication) {
	if (config.EnableWebserver) {
		app.get('/login', function (req, res) {
			...
			res.sendFile(path.dirname(process.execPath) + '/login.htm');
		});
	}
...
if (config.EnableWebserver) {
	...
	app.use('/images', express.static(path.join(path.dirname(process.execPath), './images')))
	app.use('/scripts', [isAuthenticated('/login'), express.static(path.join(path.dirname(process.execPath), '/scripts'))]);
	app.use('/', [isAuthenticated('/login'), express.static(path.join(path.dirname(process.execPath), '/custom_html'))])
}
...
try {
	for (var property in config.AdditionalRoutes) {
		if (config.AdditionalRoutes.hasOwnProperty(property)) {
			...
			app.use(property, [isAuthenticated('/login'), express.static(path.join(path.dirname(process.execPath), config.AdditionalRoutes[property]))]);
		}
	}
} catch (err) {
...
if (config.EnableWebserver) {
	app.get('/', isAuthenticated('/login'), function (req, res) {
		...
		homepageFilePath = path.join(path.dirname(process.execPath), homepageFile);
...
app.post('/sendToUE', async (req, res) => {
	console.log(JSON.stringify(req.body));
	if (req.body.type === 'multiuserControl') {
		let playerId = req.body.playerId;
		if (playerId === "" || playerId === 'Invalid Player Id') {
			if (players.size > 0) {
				hostPlayer = players.entries().next().value[0];
			}
			req.body.playerId = 'Invalid Player Id';
		} else {
		...
		sendMultiControlMsgToPlayer();
	}
	streamer.send(JSON.stringify(req.body));
});
...
streamerServer.on('connection', function (ws, req) {
	...
	ProcessManager.StreamerStarted(true);
});
...
let hostPlayer = 'Invalid Player Id';

playerServer.on('connection', async function (ws, req) {
	//兼容手动启动exe的情况
	ProcessManager.AddClicentWS(ws);
	if (!streamer) {
		ProcessManager.CreateProcess();
	}
	await ProcessManager.WaitStreamer();
	...
	function sendPlayerId() {
		for (let p of players.entries()) {
			let playerIdMsg = JSON.stringify({ type: 'playerId', id: p[0] });
			p[1].ws.send(playerIdMsg);
		}
	}

	//ue5 兼容EnableMultiuserControl配置，还可以通过页面post发送消息直接开关；
	function sendMultiControlConfig() {
		let multiControlMSG = JSON.stringify({ type: 'multiuserControl', playerId: config.EnableMultiuserControl ? 'Invalid Player Id' : hostPlayer });
		console.logColor(logging.Yellow, `<- sendMultiControlConfig: ${multiControlMSG}`);
		streamer && streamer.send(multiControlMSG);
	}
	...
	ws.on('message', function (msg) {
		...
		if (msg.type == 'offer') {
			...
			msg.playerId = playerId;
			//ue4
			msg.enableMultiuserControl = config.EnableMultiuserControl;
			streamer.send(JSON.stringify(msg));
	...
	function onPlayerDisconnected() {
		try {
			...
			streamer && streamer.send(JSON.stringify({ type: 'playerDisconnected', playerId: playerId }));
			sendPlayerDisconnectedToFrontend();
			sendPlayerDisconnectedToMatchmaker();
			sendPlayersCount();
			//当主控页面被关闭后，默认设置第一个页面为主控页面
			if (hostPlayer === playerId && players.size > 0) {
				hostPlayer = players.entries().next().value[0];
				sendMultiControlConfig();
				sendMultiControlMsgToPlayer();
			}
		} catch (err) {
...
if (config.UseMatchmaker) {
	...
	matchmaker.on('connect', function () {
		...
		matchmaker.write(JSON.stringify(message), "utf8", function () {
	...
	function connect() {
		console.log(matchmakerPort, matchmakerAddress);
		//matchmaker.setKeepAlive(true);
		matchmaker.connect(matchmakerPort, matchmakerAddress);

		/*matchmaker.connect({port: 9999, host: "localhost"} ,function(){
...
//ue5 切换主控时通知主控页面可修改分辨率；
function sendMultiControlMsgToPlayer() {
	let multiControlMSG = JSON.stringify({ type: 'multiuserControl', playerId: hostPlayer });
	console.logColor(logging.Yellow, `<- sendMultiControlMsgToPlayer: ${multiControlMSG}`);
	for (const p of players.values()) {
		p.ws.send(multiControlMSG);
	}
}
